...
export default function WorklistPage() {
  ...
  const charger = useCallback(async () => {
    setChargement(true)
    try {
      const data = await getWorklist(inclureTerminees)
      setDemandes(data)
      setPage(1)
    } catch {
      setDemandes([])
  ...
  const nombreStat = demandes.filter(d => d.urgence === 'STAT' && d.statut !== 'ANNULEE').length
  const nombreUrgente = demandes.filter(d => d.urgence === 'URGENTE' && d.statut !== 'ANNULEE').length
  ...
  return (
    <div className="space-y-6">
      <WorklistHeader
        ...
        inclureTerminees={inclureTerminees}
        onToggleTerminees={val => { setInclureTerminees(val); setPage(1) }}
      />
